...
VIDEO_FILE = BASE_DIR / "Verteilung_Erklaerung.mp4"
...
def get_http_session():
    ...
    session.headers.update({"User-Agent": "BATNA-Survey/1.0"})
    return session


def request_google_script(method, *, params=None, json_data=None, attempts=3):
    """Calls Apps Script with short automatic retries.

    GET and POST are idempotent because the same submission_id is reused.
    This means a timeout can be retried without creating a second assignment
    or a duplicate answer row.
    """
        ...
        try:
            ...
            if data.get("status") == "busy":
                raise requests.RequestException(
                    data.get("message", "Der Server ist gerade ausgelastet.")
                )
    ...
    raise RuntimeError(
        "Die Verbindung zur Datenspeicherung ist nach mehreren Versuchen fehlgeschlagen."
    ) from last_error


def get_assignment_from_google_sheet(submission_id):
    data = request_google_script(
        "GET",
        params={
            "action": "start",
...
def start_study():
    # Bei einem fehlgeschlagenen Startversuch wird dieselbe ID erneut benutzt.
    # Dadurch entsteht beim erneuten Klicken keine zweite Reservierung.
    if not st.session_state.pending_submission_id:
        st.session_state.pending_submission_id = str(uuid.uuid4())

    submission_id = st.session_state.pending_submission_id

    participant_id, phase1_distribution, phase2_distribution, combination_id = get_assignment_from_google_sheet(
        submission_id
    )

    st.session_state.phase = "intro_video"
    st.session_state.participant_id = participant_id
    st.session_state.submission_id = submission_id
    st.session_state.pending_submission_id = None
...
if st.session_state.phase == "welcome":
    ...
    st.info(
        "Hinweis: Beim Starten und beim abschließenden Speichern kann die Verbindung in seltenen Fällen kurz länger brauchen. "
        "Bitte lassen Sie die Seite geöffnet. Falls eine Fehlermeldung erscheint, klicken Sie den jeweiligen Button erneut; "
        "bereits vergebene Kombinationen und Antworten werden dabei nicht doppelt gespeichert."
    )
...
elif st.session_state.phase == "intro_video":
    st.title("Pilotstudie — Stochastische BATNA & Reservationspreis")
    st.subheader("Kurze Erklärung zu den Verteilungen")
    st.write(
        "Bitte sehen Sie sich das folgende Erklärvideo vollständig an. Anschließend können Sie mit der Umfrage fortfahren."
    )
    ...
    if VIDEO_FILE.exists():
        try:
            ...
            # Fallback für ältere Streamlit-Versionen ohne autoplay-Parameter.
            st.video(str(VIDEO_FILE), format="video/mp4")

        st.caption(
            "Falls das Video nicht automatisch startet, drücken Sie bitte im Videoplayer auf Play."
        )

        if st.button("Weiter zur Umfrage", type="primary"):
            st.session_state.scroll_token += 1
            st.session_state.phase = "first_stimulus"
            st.rerun()
    else:
        st.error(
            "Das Erklärvideo wurde nicht gefunden. Bitte informieren Sie die Versuchsleitung."
        )
